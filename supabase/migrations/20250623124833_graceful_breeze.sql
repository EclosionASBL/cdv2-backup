/*
  # Fix process_cancellation_approval function

  1. Changes
    - Update the process_cancellation_approval function to handle individual registration cancellations
    - Ensure proper type handling between UUID and TEXT parameters
    - Fix error handling and return consistent JSONB structure
...
BEGIN
  ...
  EXCEPTION WHEN invalid_text_representation THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_message', 'Invalid UUID format for request ID: ' || p_request_id
    );
  END;
  ...
  IF p_refund_type NOT IN ('full', 'partial', 'none') THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_message', 'Invalid refund type. Must be full, partial, or none.'
    );
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_message', 'Cancellation request not found'
    );
  END IF;
  ...
  IF v_request.status != 'pending' THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_message', 'Cancellation request is not in pending status'
    );
  END IF;
  ...
  IF v_refund_amount > 0 THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RETURN jsonb_build_object(
        'success', false,
        'error_message', 'Error creating credit note: ' || SQLERRM
      );
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      RETURN jsonb_build_object(
        'success', false,
        'error_message', 'Error updating cancellation request: ' || SQLERRM
      );
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      RETURN jsonb_build_object(
        'success', false,
        'error_message', 'Error updating registration status: ' || SQLERRM
      );
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      RAISE WARNING 'Error updating session count: %', SQLERRM;
    END;
