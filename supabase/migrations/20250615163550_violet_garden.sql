/*
  # Rename coda_file_imports to csv_file_imports
  
  1. Changes
    - Create csv_file_imports table if it doesn't exist
    - Copy data from coda_file_imports to csv_file_imports if the old table exists
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'coda_file_imports') THEN
    EXECUTE '
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'coda_file_imports') THEN
    EXECUTE 'DROP TABLE coda_file_imports';
  END IF;
END $$;