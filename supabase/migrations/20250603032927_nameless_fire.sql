/*
  # Update invoice sequencing to annual only

  1. Changes
    - Replace tag-based invoice sequencing with annual-only sequencing
    - Maintain the format CDV-TAG-YY00001 but sequence numbers are global per year
    - Drop old function and table, create new ones with proper signatures
...
DROP FUNCTION IF EXISTS get_next_invoice_sequence(text, date);
