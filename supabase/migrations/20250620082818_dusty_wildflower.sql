...
BEGIN
    ...
    -- First, calculate total credit notes for this invoice
    SELECT COALESCE(SUM(amount), 0) INTO v_total_credit_notes
    FROM public.credit_notes
    WHERE (invoice_id = v_invoice_id::text OR invoice_number = p_invoice_number)
    AND type = 'cancellation';
    
    -- If credit notes fully cover the invoice amount, mark as cancelled
    IF v_total_credit_notes >= v_invoice_amount THEN
        ...
        -- Exit early as the invoice is fully cancelled
        RETURN;
    END IF;
    ...
    -- Consider the remaining amount after credit notes
    IF v_total_payments >= (v_invoice_amount - v_total_credit_notes) THEN
        ...
        v_overpayment_amount := v_total_payments - (v_invoice_amount - v_total_credit_notes);
